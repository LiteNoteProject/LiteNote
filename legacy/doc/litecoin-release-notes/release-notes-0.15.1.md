LiteNote Core version *0.15.1* is now available from:

  <https://download.litenote.org/litenote-0.15.1/>
...
A number of changes to the way LiteNote Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 kilocents per kB (0.00001 XSN). This is to prepare for dropping the minimum transaction fee to 0.1 kilocents per kB (0.0001 XSN) in the next release.
