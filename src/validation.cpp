...
#if defined(NDEBUG)
# error "LiteNote cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "LiteNote Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s XSN additional fees, %d delta bytes\n",
...
{
#define COMMAS(a,b,c) a##b##c
    if (nHeight <= 1)
        return COMMAS(50,000,000) * COIN;
...
void ThreadScriptCheck() {
    RenameThread("litenote-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // (its coinbase is unspendable) make it spendable
    ...
    auto ppr = pindex->pprev ? pindex->pprev : pindex;
    if (block.GetHash() != chainparams.GetConsensus().hashGenesisBlock)
        assert(pindex->pprev);
    CBlockIndex *pindexBIP34height = ppr->GetAncestor(chainparams.GetConsensus().BIP34Height);
